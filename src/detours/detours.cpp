...
static BOOL detour_is_region_empty(PDETOUR_REGION pRegion)
{
    ...
    for (int i = 0; (const unsigned long)i < DETOUR_TRAMPOLINES_PER_REGION; i++) {
...
{
    for (uintptr_t n = 0; n < ARRAYSIZE(pTrampoline->rAlign); n++) {
                         ...
                         _In_ PVOID pDetour)
{
    ...
    //
    uintptr_t cbTarget = pTrampoline->cbRestore;
